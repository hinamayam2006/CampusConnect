...
export default function ListingDetailPage() {
  ...
  const interest = async () => {
    ...
    try {
      ...
      setListing((current) => (current ? { ...current, hasRequested: true } : current));
      setMessage('');
    } catch (err) {
    ...
    <div className="container py-4 py-md-5">
      ...
      <div className="row g-4">
        <div className="col-lg-7">
          <div
            className="bg-light rounded-4 overflow-hidden border shadow-sm d-flex align-items-center justify-content-center"
            style={{ aspectRatio: '4 / 3' }}
          >
            {listing.images?.length ? (
              <ImageCarousel images={listing.images} alt={listing.title} aspectRatio="4 / 3" />
            ) : (
            ...
            <div className="card border-0 shadow-sm rounded-4 mb-3">
              <div className="card-body p-3 p-lg-4">
                <div className="d-flex align-items-center gap-3 mb-3">
                  <div
                    className="rounded-circle bg-light border d-flex align-items-center justify-content-center fw-semibold"
              ...
              <div className="card-body p-3 p-lg-4">
                {listing.hasRequested ? (
                  <div className="alert alert-info py-2 px-3 mb-0 small rounded-3">
                    Your request has been sent and is pending review.
                  </div>
                ) : (
                  <>
                    <div className="mb-3">
                    ...
                    <button
                      type="button"
                      className={`${hubStyles.btnPrimary} w-100 justify-content-center`}
                      onClick={interest}
                      disabled={requesting}
